...
class DatabaseUtils {
     ...
     */
    static String generateForeignKey(String parent, String field) {
        String parentKey = camel2Underline(new StringBuffer(parent)).toString();
        if (parentKey.startsWith("_")) {
            parentKey = parentKey.substring(1);
        }

        String fieldKey = camel2Underline(new StringBuffer(field)).toString();
        if (fieldKey.startsWith("_")) {
            fieldKey = fieldKey.substring(1);
        }

        return fieldKey + "_" + parentKey + "_id";
    }
