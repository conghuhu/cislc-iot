...
class GeographicView extends Component {
  componentDidMount = () => {
    ...
    if (dispatch) {
      dispatch({
        type: 'accountAndAccountSettings/fetchProvince',
      });
    }
  };
  ...
  componentDidUpdate(props) {
    ...
    if (!props.value && !!value && !!value.province) {
      if (dispatch) {
        dispatch({
          type: 'accountAndAccountSettings/fetchCity',
...
export default connect(({ accountAndAccountSettings, loading }) => {
  const { province, city } = accountAndAccountSettings;
  return {
    province,
    city,
